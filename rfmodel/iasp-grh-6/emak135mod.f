...
c         discontinuities in model
      ...
      data modnam/'ak135-mod'/, first/.true./
      if (first) then
	 first = .false.
	 do 10 i=1,nd
	    rd(i) = re-rd(i)
...
C     Alternative model is to decrease speeds 1% downward from D'' boundary.
      if (pz .gt. 2740.00 .and. pz .lt.  2891.50) then
         frac = (pz-2740.0)/(2891.5-2740.0)
         pp = pp*(1.0 - 0.0585*frac)
         ps = ps*(1.0 - 0.0585*frac)
      endif
      end
