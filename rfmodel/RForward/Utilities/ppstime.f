...
*     program to calculate the P - Ps traveltime through a plane
*     layered model.  Vertical incidence is assumed so the time
*     difference through a layer is just h/beta - h/alpha
*        based on program vplot
*
     ...
     *            dum1,dum2,strike,dip,-1,ier)
      do 2 ik=1,1
      	...
      	do 1 i=1,nlyrs-1
      		...
      		v2 = p(i+1,ik)
      		ij = ij+1
      		t(ij) = t(ij-1)+h(i)*(1/p(i,1) - 1/p(i,2))
		r(ij) =0.
      		t(ij+1) = t(ij)
      		r(ij+1) = v2 - v
		t(ij+2) = t(ij)
