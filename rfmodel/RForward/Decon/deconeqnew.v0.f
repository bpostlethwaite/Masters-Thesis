...
C     Modified to use multiple-window with overlap DFT to (attempt to) preserve
C     spectral for long time series (e.g. deconvolution for transition zone
C     receiver function work).
C
...
C
C     - fixed bug in power equalization between decon wavelet window and
C       trace lengths --GH 8 Jan 2003 
C     - multiple-window DFT with overlap --GH 10 Jan. 2003

      program pwaveqn
      parameter (npmax=30000,npow=12,ndim=2**npow, aztol=0.001)
      logical ex,debug
      real se(npmax),sn(npmax),s(npmax,3)
      real d2(ndim),dw(ndim),tap(ndim),camp(1+ndim/2),cphs(1+ndim/2)
      complex cval,data(ndim/2,3),wdata(ndim/2)
      equivalence (se,d2),(sn,tap),(wdata,dw)
...
c       from this.
     ...
     +         'quitting.'
      ...
      enddo
      call rsach(line,nerr)
      if (nerr .ne. 0) stop '**Can''t read SAC file.'
c     call getnhv('NZYEAR',iyr,nerr)
c     call getnhv('NZJDAY',iday,nerr)
c     call getnhv('NZHOUR',ihr,nerr)
c     call getnhv('NZMIN',imin,nerr)
c     call getnhv('NZSEC',isec,nerr)
c     call getnhv('NZMSEC',imsec,nerr)
c     sec = isec + float(imsec)*1e-3
c     Accumulate event information.
...
c     Read in start time relative to file zero and name of phase.
     ...
     +    loc1 .lt. 1 .or. loc2 .lt. 1 .or. nloc .gt. ndim) 
...
c        same storage, real array overlaying complex one.
	 ...
	 f = de + (t-tloc2)*fac
	 if (t .le. tloc2) then
	    val = wigint(beg,s(1,3),npts,dt,1e-4,t) - f
	 else
	    val = 0.0
	 endif
	 dw(i) = val*tap(i)
...
c     Shift data for easy FFT, calculate window function.
      nout = 1 + npts-loc1
      nift = npow2(nout)
      delfi = 2*fny/nift
      if (loc1 .ne. 1) then
	 do i=1,3
	    call tscopy(s(loc1,i),s(1,i),nout)
	 enddo
      endif

c     FFT
      do i=1,3
         call rlovft(nout,nloc,s(1,i),tap,data(1,i),d2)
      enddo
      ...
      do j=2,nfpts-1
	 val = real(wdata(j)*conjg(wdata(j)))
...
c        filter
     ...
     &           * exp(cmplx(0.,w*tdelay))
	    ...
	    camp(j) = abs(cval)
	    cphs(j) = atan2(imag(cval),real(cval))
	 enddo
	 phi = max(phi1,d2(1))
	 camp(1) = real(data(1,i))*real(wdata(1)) / phi
	 cphs(1) = 0.0
	 phi = max(phi1,d2(nfpts))
	 w = 2*pi*fny
	 gauss = -(w/agauss)**2/4.0
	 cval = imag(data(1,i))*imag(wdata(1)) * exp(gauss)/phi
     &           * exp(cmplx(0.,w*tdelay))
         camp(nfpts) = abs(cval)
	 cphs(nfpts) = atan2(imag(cval),real(cval))
	 if (i .eq. 3) gnorm = gnorm + 1 + exp(gauss)
	 call itspec(fny,nfpts,camp,cphs,nift/2,s(1,i))
	 if (debug) then
	    tap(1)=abs(s(1,i))
	    do j=2,nift/2
	       k=2*(j-1)
	       tap(j)=sqrt(s(1+k,i)**2 + s(2+k,i)**2)
	    enddo
	    tap(nift/2+1)=abs(s(2,i))
	    call setfhv('B',0.0,nerr)
	    call setnhv('NPTS',nift/2+1,nerr)
	    call setfhv('DELTA',delfi,nerr)
	    write(line,'(a,1h.,i1,a)') sacpfx(1:lsacpf),i,'.spec'
	    call wsac0(line,tap,tap,nerr)
	    call setfhv('B',0.0,nerr)
	    call setnhv('NPTS',nfpts,nerr)
	    call setfhv('DELTA',fny/(nfpts-1),nerr)
	    write(line,'(a,1h.,i1,a)') sacpfx(1:lsacpf),i,'.spea'
	    call wsac0(line,camp,camp,nerr)
	    write(line,'(a,1h.,i1,a)') sacpfx(1:lsacpf),i,'.spep'
	    call wsac0(line,cphs,cphs,nerr)
	 endif
         call realft(s(1,i),nift,-1)
      enddo
...
c-
	 ...
	 AR(I) = AR_temp 
      ...
      subroutine itspec(fny,nfin,amp,phs,nfout,spec)
      PARAMETER (PI=3.141592653589793D0,PI2=6.28318530717959D0)
      real amp(nfin), phs(nfin)
      complex spec(nfout)

      dfi = fny/(nfin-1)
      dfo = fny/nfout

C     Unwrap phase, and unkink.  Unkinking needed because this
C        prevents phantom phase arrivals due to interpolation of phase
C        spectrum between points which have different phase ramps.

      call phunw(phs,nfin)
      pimult = 0
      if (phs(2) .lt. 0) phs(2) = phs(2) - pi2*(int(phs(2)/pi2)-1)
      do k=3,nfin
	 i = k - 2
	 j = k - 1
	 pred = 2*phs(j) - phs(i)
	 npi = int((phs(k)+pimult-pred)/pi)
C        ***THIS CODE SEEMS TO BE WRONG*** Can't figure out what it should do.
	 if (npi .ne. 0) then
	    pimult = pimult - (npi/2)*pi2
	 endif
         phs(k) = phs(k) + pimult
      enddo

C     First point is special - contains f(0) and f(ny) in real & imag parts.
C     Both are real for a real time series.
      spec(1) = cmplx(amp(1),amp(nfin))

C     Sample between points 
      nm = nfout/(nfin-1) - 1
      dm = 1./(1+nm)
C     alo = log(amp(1))
      alo = amp(1)
...
C        endif
	 ...
	 pdf = phi-plo
         ...
         do j=1,nm
	    f = j*dm
C           camp = exp(alo + f*adf)
	    camp = alo + f*adf
	    cphs = plo + f*pdf
	    spec(k) = camp*exp(cmplx(0.,cphs))
	    k = k + 1
	 enddo
	 alo = ahi
	 plo = phi
	 if (i .ne. nfin) spec(k) = amp(i)*exp(cmplx(0.,phi))
      enddo
      end

      SUBROUTINE PHUNW (PHZ,LPHZ)
      DIMENSION PHZ(LPHZ)
      PARAMETER (PI=3.141592653589793D0,PI2=6.28318530717959D0)
