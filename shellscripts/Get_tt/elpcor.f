...
c This routine is a MATLAB MEX gateway from MATLAB to the 
c travel time ellipticity correction subroutine
c written 28 May 1991 by David Taylor, ENSCO, Inc.



c  USAGE: [ecorr,ier] = elpcor(phase,delta,depth,azim,eqcolat);
c  Calculate ellipticity travel time corrections
c  
c INPUT:
c   phase   - Phase names ( P PcP PKPab PKPbc PKPdf S ScS 
c                           SKSac SKSdf Scp SKP )
c   delta   - Epicenter to receiver, distances in degrees.
c   depth   - Source depths, in km.
c   azim    - Epicenter to receiver azimuths, in degrees.
c   eq_colat- Epicenter colatitudes, in degrees.
c  
c delta, depth, azim, and eq_colat can be column vectors or 
c scalars.  If they are vectors they must have the same number 
c of rows, or be scalars phase may be either one phase name, 
c or a series of phase names separated by blank spaces and 
c contained in a row vector:  eg. 
c phase = 'P PcP PKPab P P PcP'; delta=(70:77)';
c depth=645;azim=(180:10:250)';eq_colat=134;
c  
c OUTPUT:
c   ecorr   - ellipticity correction (s) to be added to travel 
c             time calculated for spherically symmetric model
c   ier     = 0 if no error
...
c  
c ecorr and ier are column vectors


      save
      integer dim
      parameter (dim=5000)
      double precision rtp1(dim*10),rtp2(dim),rtp3(dim),rtp4(dim),
     .                 rtp5(dim),ecorrd(dim),ierd(dim)
      integer i,k,m,lenphs,nphs,ierr,ier,istart(dim),iend(dim)
      character*50000 phase
      character*8 phid, string*1
      logical  blank0, blank1
      integer m2,m3,m4,m5,n1,n2,n3,n4,n5
      real depth,delta,azim,ecolat,ecorr
...
c
      if (nrhs .eq. 0) then
        write(0,'(30(a/))')
     .' USAGE: [ecorr,ier] = elpcor(phase,delta,depth,azim,eq_colat);',
     .' Calculate ellipticity travel time corrections',
     .' ',
     .'INPUT:',
     .'  phase   - Phase names ( P PcP PKPab PKPbc PKPdf S ScS ',
     .'                          SKSac SKSdf Scp SKP )',
     .'  delta   - Epicenter to receiver, distances in degrees.',
     .'  depth   - Source depths, in km.',
     .'  azim    - Epicenter to receiver azimuths, in degrees.',
     .'  eq_colat- Epicenter colatitudes, in degrees.',
     .'',
     .'delta, depth, azim, and eq_colat can be column vectors or ',
     .'scalars.  If they are vectors they must have the same number ',
     .'of rows, or be scalars phase may be either one phase name, ',
     .'or a series of phase names separated by blank spaces and ',
     .'contained in a row vector:  eg. ',
     .'phase = ''P PcP PKPab P P PcP''; delta=(70:75)'';',
     .'depth=645;azim=(180:10:230)'';eq_colat=134;'
        write(0,'(30(a/))')
     .'OUTPUT:',
     .'  ecorr   - ellipticity correction (s) to be added to travel ',
     .'            time calculated for spherically symmetric model',
     .'  ier     = 0 if no error',
     .'            1 if corrections do not exist for desired phase',
     .'            2 if corrections do not exist for desired distance',
     .'            3 if corrections do not exist for desired depth',
     .' ',
     .'ecorr and ier are column vectors'

        return
      else if (nrhs .ne. 5) then
        call mexErrMsgTxt('ELPCOR requires five input arguments')
      end if
      
c interpret first input argument (phase)

c assign pointer to rhs argument and convert double
c precission numbers to character string
      
      call mxCopyPtrToReal8(mxGetPr(prhs(1)),rtp1,
     .                      mxGetM(prhs(1))*mxGetN(prhs(1)))
      n1=mxGetN(prhs(1))
c     this loop converts that double precision row vector to an
c     equivalent character string and removes leading blanks, and 
c     all double blanks
      k=0
      blank0=.true.
      do 1, i=1,n1
        string=char(int(rtp1(i)))
        if (string.eq.' ') then 
          blank1=.true.
        else
          blank1=.false.
        ...
        if (.not. blank0 .or. .not. blank1) then
          k=k+1
          phase(k:k)=string
        end if
        blank0 = blank1
...
c     remove trailing blanks
      do 2, i=k,1,-1
        if (phase(i:i).ne.' ') goto 3
2     continue
3     lenphs=i
      k=1
      istart(1)=1
      do 4, i=1,lenphs
        if (phase(i:i).eq.' ') then
          iend(k)=i-1
          k=k+1
          istart(k)=i+1
        end if
...
c interpret second input argument (delta) should be a column vector
      call mxCopyPtrToReal8(mxGetPr(prhs(2)),rtp2,
     .                      mxGetM(prhs(2))*mxGetN(prhs(2)))
      m2=mxGetM(prhs(2))
      n2=mxGetN(prhs(2))

c interpret third input argument (depth) should be a column vector
      call mxCopyPtrToReal8(mxGetPr(prhs(3)),rtp3,
     .                      mxGetM(prhs(3))*mxGetN(prhs(3)))
      m3=mxGetM(prhs(3))
      n3=mxGetN(prhs(3))

c interpret fourth input argument (azimuth) should be a column vector
      call mxCopyPtrToReal8(mxGetPr(prhs(4)),rtp4,
     .                      mxGetM(prhs(4))*mxGetN(prhs(4)))
      m4=mxGetM(prhs(4))
      n4=mxGetN(prhs(4))

c interpret fifth input argument (event colatitude) should be a column vector
      call mxCopyPtrToReal8(mxGetPr(prhs(5)),rtp5,
     .                      mxGetM(prhs(5))*mxGetN(prhs(5)))
      ...
      m=max(m,m4)
      m=max(m,m5)
      if     (nphs.ne.1 .and. nphs.ne.m) then  
        ierr=1
        call mexErrMsgTxt('ELPCOR: wrong number of phase names')
      else if (m2  .ne.1 .and. m2  .ne.m) then  
        ierr=2
        call mexErrMsgTxt('ELPCOR: wrong number of deltas')
      else if (m3  .ne.1 .and. m3  .ne.m) then  
        ierr=3
        call mexErrMsgTxt('ELPCOR: wrong number of depths')
      else if (m4  .ne.1 .and. m4  .ne.m) then  
        ierr=4
        call mexErrMsgTxt('ELPCOR: wrong number of azimuths')
      else if (m5  .ne.1 .and. m5  .ne.m) then  
        ierr=5
        call mexErrMsgTxt('ELPCOR: wrong number of event colats')
      else if (n2.gt.1) then
        ierr=2
        call mexErrMsgTxt('ELPCOR: delta must be a column vector')
      else if (n3.gt.1) then
        ierr=3
        call mexErrMsgTxt('ELPCOR: depth must be a column vector')
      else if (n4.gt.1) then
        ierr=4
        call mexErrMsgTxt('ELPCOR: azimuth must be a column vector')
      else if (n5.gt.1) then
        ierr=5
        call mexErrMsgTxt('ELPCOR: eqcolat must be a column vector')
      else
        ierr=0
      end if
...
c     call Elipticity correction subroutine
      do 10, i=1,m
        if (nphs.eq.m) k     =i
        if (m2  .eq.m) delta =real(rtp2(i))
        if (m3  .eq.m) depth =real(rtp3(i))
        if (m4  .eq.m) azim  =real(rtp4(i))
        if (m5  .eq.m) ecolat=real(rtp5(i))
        phid='        '
        phid(1:iend(k)-istart(k)+1)=phase(istart(k):iend(k))
        call elpcr(phid,delta,depth,azim,ecolat,ecorr,ier)
        ecorrd(i)=ecorr
        ierd(i)  =ier
c       write(*,*) phid,delta,depth,azim,ecolat,ier,ecorr
10    continue
...
c first output argument is ellipticity correction

         plhs(1)=mxCreateFull(m,1,0)
         call mxCopyReal8ToPtr(ecorrd, mxGetPr(plhs(1)), m)

c second output argument is error 

         plhs(2)=mxCreateFull(m,1,0)
         call mxCopyReal8ToPtr(ierd, mxGetPr(plhs(2)), m)
	...
	subroutine elpcr(phid,del,z,azi,ecolat,ecorr,ierr)
c
c
...
c
	if (phid(1:2).eq.'P ' .or. phid(1:3).eq.'Pn ' .or. phid(1:3).eq.'Pg ' 
     ...
     +		.or. phid(1:3).eq.'Sb ' .or. phid(1:3).eq.'Sdi') then
	  ips = 6
