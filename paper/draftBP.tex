...
\begin{figure}
  ...
  \caption{The average of all station depth profiles (black) and the SVD singular vectors of the station depth profile matrix. Each vector represents a mode that captures some part of the total variance in the dataset. Vertical lines are the axis each profile and vector is normalized about. A grey horizontal window highlights a region of low energy.}
  \label{fig:conradPCA}
\end{figure}


Another feature of interest is the average crustal structure across Canada. We perform a stack along $H$ using the best estimate for $R$ computed via the ZK + semblance method given above. This depth profile contains energy at depths corresponding to velocity discontinuities. Accumulating all station depth profiles into a matrix we then decompose the data via SVD into its major modes. Figure \ref{fig:conradPCA} contains the average depth profile for all stations and the first five major singular vectors or modes accounting for 78\% of the variance in the data. Additional modes have not been included as mode six contains energy at similar depths to modes two and three while modes seven and above appear noisy and represent variances in the dataset that do not account for significant variance or structure in the dataset. Each vector is normalized to its maximum for clarity and centred on an axis given as vertical grey lines. The first profile (black) is the average of all stations depth stacks. The second mode capturing 50\% of the variance in the dataset represent the mean and as expected appears similar to the average depth stack. The second and third modes are dominated by a shallow discontinuity and a deeper discontinuity associated with sedimentary layers and the Moho respectively. The fourth and fifth modes, accounting for a combined 11\% of the variance in the dataset, contain energy at upper-mid crustal depths in the 10km to 20km range. All modes display lower energy within a window roughly stretching from 20km depth to 30km.
