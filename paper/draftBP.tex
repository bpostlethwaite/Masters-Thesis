...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{discontinuities}
  \caption{}
  \label{fig:disc}
\end{figure}
...
Seismic reflectivity of the continental crust is a valuable constraint on the processes governing crustal evolution. Early efforts to characterize continental crust revolved around a slab model with a lower velocity upper crust and a higher velocity lower crust [Conrad, 1925; Jeffreys, 1926]. Evidence for this model was originally put forth by Conrad [1925] who suggested observable P arrivals were propogating along a mid-crustal interface similar to the deeper crust-mantle interface imaged by Andrija Mohorovičić [Litak and Brown, 1989]. Both the crustal slab model and the Conrad Discontinuity, as the mid-crustal interface became known, persisted in the literature but were increasingly criticized as being too simplistic, in the case of the slab model, or, for the Conrad Discontinuity, lacking consistent evidence [Litak and Brown, 1989]. The development of reflection seismology allowed workers to further argue for a complex heterogenous crust and propose a model whereby smooth random lateral inhomogenities superimposed on a linearly increasing velocity gradient could produce seismic responses similar to that of a simple two layer model [Litak and Brown, 1989; Mereu and Ojo, 1981].

Despite the fading support for the Conrad Discontinuity as a discrete geological phenomena the generalization of the crust into seismically distinguishable upper and lower components continued. Mooney and Brocher [1987] argued for a model of the continental crust seperated into a seismically reflective lower crust overlain by transparent upper crust. Their physical interpretion is of a poorly laminated upper crust dominated by features of short length scale contrasted with a lower crust composed of thin (100 metre) lamellae of significant length scales. Supporting evidence for a reflective lower crust [Ito, 1998; Mayer, 1996; Meissner et. al., 2006] indicate that this is likely regional and representational of extented as well as thicker warmer crusts.
...
The lower energy noted in Section \ref{section:discontinuities} in the 20 to 30 km depth window for the station profiles given in figure \ref{fig:disc} provides some evidence for seismically distinguishable crustal layers. To investigate this further we stretch and interpolate the profiles so that the Moho of each profile lines up on a common axis (Figure \ref{fig:discInterp}, left) and then take the magnitude of each profile and stack (Figure \ref{fig:discInterp}, right). The energy of the stack is most pronounced near the surface, associated with sedimentary layers, and at the Moho. There is a marked decrease in energy in the bottom third of the profile.

(Jackson 2002 - strength of lithosphere - mirrors decrease in energy)
