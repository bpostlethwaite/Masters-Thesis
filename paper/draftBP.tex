...
Seismic reflectivity of the continental crust is a valuable constraint on the processes governing crustal evolution. Early efforts to characterize continental crust revolved around a slab model with a lower velocity upper crust and a higher velocity lower crust [Conrad, 1925; Jeffreys, 1926]. Evidence for this model was originally put forth by Conrad [1925] who suggested observable P arrivals were propagating along a mid-crustal interface similar to the deeper crust-mantle interface imaged by Andrija Mohorovičić [Litak and Brown, 1989]. Both the crustal slab model and the Conrad Discontinuity, as the mid-crustal interface became known, persisted in the literature but were increasingly criticized as being too simplistic, in the case of the slab model, or, for the Conrad Discontinuity, lacking consistent evidence [Litak and Brown, 1989]. The development of reflection seismology allowed workers to further argue for a complex heterogeneous crust and propose a model whereby smooth random lateral inhomogeneities superimposed on a linearly increasing velocity gradient could produce seismic responses similar to that of a simple two layer model [Litak and Brown, 1989; Mereu and Ojo, 1981].

Despite the fading support for the Conrad Discontinuity as a discrete geological phenomena the generalization of the crust into seismically distinguishable upper and lower components continued. Mooney and Brocher [1987] argued for a model of the continental crust separated into a seismically reflective lower crust overlain by transparent upper crust. Their physical interpretation is of a poorly laminated upper crust dominated by features of short length scale contrasted with a lower crust composed of thin (100 metre) lamellae of significant length scales. Supporting evidence for a reflective lower crust confirms widespread laminated structure in the lower crust with thicknesses of 100m to 300m though these structures have been found to be localized and representational of extended as well as thicker warmer crusts [Ito, 1998; Mayer, 1996; Meissner et. al., 2006].
...
The lower energy noted in Section \ref{section:discontinuities} in the 20km to 30km depth window for the station profiles given in figure \ref{fig:disc} provides some evidence for seismically distinguishable crustal layers. To investigate this further we stretch and interpolate the profiles so that the Moho of each profile lines up on a common axis (Figure \ref{fig:discInterp}, left) and then take the magnitude of each profile and stack (Figure \ref{fig:discInterp}, right). The energy of the stack is most pronounced near the surface, associated with sedimentary layers, and at the Moho. There is a marked decrease in energy in the bottom third of the profile. This study employs S-wave energy using both direct and reflected arrivals and it has been shown that these waves interact with velocity gradients on scales lengths in the order of $\lambda / 2$ for direct phases and $\lambda / 4$ for reflected phases where $\lambda$ is wavelength [Bostock, 1999]. For 1Hz teleseismic S-waves crustal structure in the scale lengths of 750m to 1500m is resolvable. Higher frequency active source data (10Hz - 40Hz) employing P-wave sources would be sensitive to structure roughly in the ranges of 75m to 300m. Lithoprobe transects [Hammer et. al., 2010] show significant heterogeneity throughout the crust including several transects, such as the Western Superior transect, that appear to contain higher reflectivity in the lower crust. One interpretation for these data, taken together, is that in general the Canadian lower crustal structure contains fewer thick (~1km) velocity contrasts as compared to the upper crust. This interpretation allows for high reflectivity in the lower crust arising from thinner, less than 750m, contrasts.

Though the Conrad Discontinuity continues to be invoked, for example in a study of the Rhine Graben [Mayer, 1996], as terminology to describe the interface between a transparent upper crust and reflective lower crust, it does not appear to be a prevalent feature of the Canadian crust. The results from this study do suggest however, that in general, and with the seismic methods employed here, a distinction between the seismic properties of the upper crust and the bottom third of the crust can be resolved. Studies on crustal strength [Jackson, 2002] and on brittle and ductile models [Burov and Watts, 2006; Meisner and Rabbel, 1999] as well as the widely noted reduced seismicity of the lower crust indicate changes in crustal processes at similar depths, though see [Deichmann, 1991; Handy and Brun, 2004; Simpson, 1999] for competing evidence.
